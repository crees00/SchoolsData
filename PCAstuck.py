...
def doPCA(numPCs = 6):
    ...
    newDFwithPCs = pd.DataFrame(data = PCs, columns = ['principal component '+str(x) for x in list(range(1,numPCs+1))])
    ...
    ax.scatter(newDFwithPCs.loc[:, 'principal component 1']
...
def doClustering():
    ...
    for numClusters in [2,3,4,5,6,7]:
        ...
        finalDF = pd.concat([finalDF, pd.Series(a, name=f'Cluster{numClusters}')], axis = 1)
        fig = plt.figure(figsize = (8,8))
        ax = fig.add_subplot(1,1,1) 
        ax.set_xlabel('Principal Component 1', fontsize = 15)
        ax.set_ylabel('Principal Component 2', fontsize = 15)
        ...
        for target, colour in zip(targets,colours[:numClusters]):
            ...
            ax.scatter(newDFwithPCs.loc[indicesToKeep, 'principal component 1']
    ...
    for numClusters in range(2,8):
        ...
        #    ax.set_ylabel('Principal Component 2', fontsize = 15)
            ...
            for target, colour in zip(targets,colours[:numClusters]):
                ...
                ax.hist(dfDataPCsClusters.loc[indicesToKeep, col],
                        alpha = 0.8,
                        rwidth = 0.8
    #                    histtype = 'step'
    #                       c=colour,
