...
def fixCategoricalCols(df):
    ...
    df['BoardingNew'] = np.where(df['Boarders (name)'].isin(['Boarding school',
       "Children's home (Boarding school)",
       'College / FE residential accomodation']),1,0)
    # 1 for 6th form, 0 for not
    df['SixthFormNew'] = np.where(df['OfficialSixthForm (name)'].isin(['Has a sixth form']),1,0)
    df['HasBoysNew'] = np.where(df['Gender (name)'].isin(['Mixed','Boys']),1,0)
    df['HasGirlsNew'] = np.where(df['Gender (name)'].isin(['Mixed','Girls']),1,0)
    df[['PNUMEAL','PNUMFSM','PERCTOT']] = df[['PNUMEAL','PNUMFSM','PERCTOT']].apply(lambda x: x/100)
...
dfForModelModified = normalise(dfForModelModified, ['Pupil:     Teacher Ratio','Mean Gross FTE Salary of All Teachers (Â£s)'])
